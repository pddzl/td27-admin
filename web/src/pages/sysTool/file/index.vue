...
import type { fileDataModel } from "@/api/sysTool/file"
import { usePagination } from "@@/composables/usePagination_n"
import { formatDateTime } from "@@/utils/datetime"
import { reactive, ref } from "vue"
import { fileDeleteApi, fileDownloadApi, fileListApi } from "@/api/sysTool/file"
...
function handleDownload(fileName: string) {
  fileDownloadApi({ name: fileName }).then((res) => {
...
function handleDelete(row: fileDataModel) {
  ElMessageBox.confirm("此操作将永久删除该文件, 是否继续?", "提示", {
    ...
    .then(async () => {
      const res = await fileDeleteApi({ name: row.fileName })
      if (res.code === 0) {
