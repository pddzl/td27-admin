...
function handleSelectionChange(val: ApiDataModel[]) {
...
function operateAction(formEl: FormInstance | undefined) {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (oKind === "Add") {
        const res = await apiCreateApi({ ...opFormData })
        if (res.code === 0) {
          ElMessage({ type: "success", message: res.msg })
          tableData.value.push(res.data)
        }
      } else if (oKind === "Edit") {
        const res = await apiUpdateApi({ id: activeRow.id, ...opFormData })
        if (res.code === 0) {
          ElMessage({ type: "success", message: res.msg })
          // 修改对应数据
...
// 删除api
function handleDeleteApi(row: ApiDataModel) {
  ElMessageBox.confirm("此操作将永久删除所有角色下该api, 是否继续?", "提示", {
    ...
    .then(() => {
      apiDeleteApi({ id: row.id }).then((res) => {
        if (res.code === 0) {
          ElMessage({ type: "success", message: res.msg })
...
async function onDelete() {
  ...
  const res = await apiDeleteByIdsApi({ ids: ids.value })
  if (res.code === 0) {
    ElMessage({
      type: "success",
      message: res.msg
