...
const tableData = ref<ListData<DictModel[]>>({ list: [], total: 0, page: 0, pageSize: 0 })
...
<template>
  <div class="app-container">
        ...
        <el-table :data="tableData.list">
          ...
          <el-table-column label="操作">
            <template #default="scope">
              <el-button type="primary" text icon="Edit" size="small" @click="editDialogHandle(scope.row)">
                编辑
              </el-button>
              <el-button type="danger" text icon="Delete" size="small" @click="deleteHandle(scope.row)">
                删除
              </el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" :before-close="handleClose" width="30%">
      <el-form ref="formRef" :model="opFormData" :rules="addFormRules" label-width="80px">
        <el-form-item label="中文名称" prop="cn_name">
          <el-input v-model="opFormData.cn_name" placeholder="请输入字典名（中）" />
        </el-form-item>
        <el-form-item label="英文名称" prop="en_name">
          <el-input v-model="opFormData.en_name" placeholder="请输入字典名（英）" :disabled="oKind === operationEnum.UPDATE" />
        </el-form-item>
      </el-form>
      <template #footer>
