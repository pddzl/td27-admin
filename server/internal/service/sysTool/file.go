...
// Upload 上传文件
func (fs *FileService) Upload(file *multipart.FileHeader) (*sysTools.FileModel, error) {
	var uploadModel sysTools.FileModel
...
func (fs *FileService) GetFileList(params sysTools.FileSearchParams) ([]sysTools.FileModel, int64, error) {
	limit := params.PageSize
	offset := params.PageSize * (params.Page - 1)
	db := global.TD27_DB.Model(&sysTools.FileModel{})
	var fileList []sysTools.FileModel
...
func (fs *FileService) Delete(fileName string) (err error) {
	...
	// 删除数据库记录
	if err == nil {
		err = global.TD27_DB.Where("file_name = ?", fileName).Delete(&sysTools.FileModel{}).Error
	}
