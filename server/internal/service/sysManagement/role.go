...
func (s *RoleService) Create(req *sysManagement.RoleModel) (*sysManagement.RoleModel, error) {
	...
	global.TD27_LOG.Info("创建角色成功，默认API权限已通过统一权限表管理", "roleId", create.ID)
	return create, err
}
...
func (s *RoleService) Delete(id uint) error {
	...
	if err = s.roleRepository.DeleteRoleMenu(s.ctx, id); err != nil {
		global.TD27_LOG.Warn("删除role权限关联失败", "error", err)
	}

	// 重新加载Casbin策略
	go func() {
		if err = casbinService.ReloadPolicy(); err != nil {
			global.TD27_LOG.Error("重新加载Casbin策略失败", "error", err)
		}
	}()
